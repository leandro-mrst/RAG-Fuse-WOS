...
class PromptOptimizerHelper(Helper):
    def __init__(self, params):
        ...
        self.session = aioboto3.Session() #TODO we are going to use vLLM instead of Bedrock
    ...
    def run(self):
        #TODO since vLLM run synchronously we do not need this anymore
        asyncio.run(
            self._run()
        )

    async def _run(self):
        #TODO this became the run method without the async prefix
